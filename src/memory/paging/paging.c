...
{
    // map directory entry and initialize to zero.
...
int paging_set(uint32_t* directory, void* virt, uint32_t val)
{
    ...
    int res = paging_get_indices(virt, &directory_index, &table_index);
    if (res < 0)
    {
        return res;
    }
    ...
    uint32_t* table = (uint32_t*) (entry & 0xfffff000); // extract only the physical address
